...
class BaseTrainer:
        ...
        for batch_idx, batch in enumerate(
            ...
            if batch_idx % self.log_step == 0:
                ...
                self.writer.add_scalar(
                    "learning rate discriminator", self.lr_scheduler[0].get_last_lr()[0]
                )
                self.writer.add_scalar(
                    "learning rate generator", self.lr_scheduler[1].get_last_lr()[0]
                )
    ...
    def _save_checkpoint(self, epoch, save_best=False, only_best=False):
        ...
        state = {
            ...
            "optimizer_disc": self.optimizer[0].state_dict(),
            "optimizer_gen": self.optimizer[1].state_dict(),
            "lr_scheduler_disc": self.lr_scheduler[0].state_dict(),
            "lr_scheduler_gen": self.lr_scheduler[1].state_dict(),
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        if (
            ...
            self.optimizer[0].load_state_dict(checkpoint["optimizer_disc"])
            self.lr_scheduler[0].load_state_dict(checkpoint["lr_scheduler_disc"])
            self.optimizer[1].load_state_dict(checkpoint["optimizer_gen"])
            self.lr_scheduler[1].load_state_dict(checkpoint["lr_scheduler_gen"])
